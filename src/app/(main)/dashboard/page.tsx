...
async function getDashboardStats(userId: string) {
  ...
  const [orderCount, userRecord, recentOrders] = await Promise.all([
    ...
    Order.find({ user: userId }).sort({ createdAt: -1 }).limit(2).select("_id orderNumber orderStatus total createdAt").lean()
  ]);
         ...
         <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="space-y-2">
               ...
               <p className="text-slate-500 font-medium tracking-tight max-w-md">
                 Here is an overview of your account activities. Manage your profile, track upcoming orders, and review saved items.
               </p>
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-2 space-y-6">
             <div className="flex flex-wrap items-center justify-between">
                <h2 className="text-lg font-black tracking-tight text-slate-900">Quick Navigation</h2>
             </div>
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                 ...
                 <Link href="/dashboard/orders" className="group">
                   <div className="h-full bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 relative overflow-hidden flex flex-col items-start gap-6">
                     ...
                     <div className="space-y-1">
                       <h3 className="text-xl font-black tracking-tight text-slate-900 group-hover:text-primary transition-colors">My Orders</h3>
                       <p className="text-sm font-bold text-slate-400">Track & manage orders</p>
                     </div>
                 ...
                 <Link href="/dashboard/wishlist" className="group">
                   <div className="h-full bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm hover:shadow-xl hover:shadow-rose-500/5 transition-all duration-300 relative overflow-hidden flex flex-col items-start gap-6">
                     ...
                     <div className="space-y-1">
                       <h3 className="text-xl font-black tracking-tight text-slate-900 group-hover:text-rose-500 transition-colors">Wishlist</h3>
                       <p className="text-sm font-bold text-slate-400">Your saved products</p>
                     </div>
                 ...
                 <Link href="/dashboard/addresses" className="group">
                   <div className="h-full bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm hover:shadow-xl hover:shadow-emerald-500/5 transition-all duration-300 relative overflow-hidden flex flex-col items-start gap-6">
                     ...
                     <div className="space-y-1">
                       <h3 className="text-xl font-black tracking-tight text-slate-900 group-hover:text-emerald-500 transition-colors">Addresses</h3>
                       <p className="text-sm font-bold text-slate-400">Manage shipping info</p>
                     </div>
                 ...
                 <Link href="/dashboard/profile" className="group">
                   <div className="h-full bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm hover:shadow-xl hover:shadow-violet-500/5 transition-all duration-300 relative overflow-hidden flex flex-col items-start gap-6">
                     ...
                     <div className="space-y-1">
                       <h3 className="text-xl font-black tracking-tight text-slate-900 group-hover:text-violet-500 transition-colors">Profile</h3>
                       <p className="text-sm font-bold text-slate-400">Update account settings</p>
                     </div>
         ...
         <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between">
               ...
               <Link href="/dashboard/orders" className="text-xs font-black text-primary hover:underline underline-offset-4 decoration-2">
                 View All
               </Link>
            </div>
            ...
            <div className="bg-white rounded-[2rem] border border-slate-200 p-6 shadow-sm overflow-hidden flex flex-col gap-4">
               {stats.recentOrders.length > 0 ? (
                 stats.recentOrders.map((order) => {
                    const StatusIcon = STATUS_ICONS[order.orderStatus || 'pending'] || Clock;
                    return (
                      <Link key={order._id} href={`/dashboard/orders/${order._id}`} className="group relative block p-4 rounded-2xl hover:bg-slate-50 transition-colors border-b border-dashed border-slate-100 last:border-0 last:pb-0">
                         <div className="flex items-start justify-between gap-4">
                            <div className="flex items-center gap-3">
                               <div className={cn("size-10 rounded-xl flex items-center justify-center shadow-sm border", STATUS_COLORS[order.orderStatus || 'pending'])}>
                                  <StatusIcon className="size-5" />
                               </div>
                               <div>
                                  <p className="font-black text-sm text-slate-900 tracking-tight group-hover:text-primary transition-colors">#{order.orderNumber}</p>
                                  <p className="text-xs font-bold text-slate-400">{format(new Date(order.createdAt), "MMM dd, yyyy")}</p>
                               </div>
                            </div>
                            <div className="text-right">
                               <p className="font-black text-sm text-slate-900">{formatPrice(order.total)}</p>
                               <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{order.orderStatus}</span>
                            </div>
                         </div>
                      </Link>
                    )
                 ...
                 <div className="text-center py-12 px-4">
                    ...
                    <p className="text-xs font-medium text-slate-500 mt-1">When you place orders, they will appear here.</p>
                 </div>
               )}
            </div>

             {/* Payment History Teaser */}
            <Link href="/dashboard/payments" className="block mt-4 relative overflow-hidden bg-slate-900 text-white rounded-[2rem] p-6 hover:shadow-lg transition-all group">
               ...
               <div className="flex items-center gap-4 relative z-10">
                 ...
                 <div className="pr-8">
                   <h3 className="font-black tracking-tight text-white group-hover:text-amber-400 transition-colors">Payment History</h3>
                   <p className="text-slate-400 font-medium text-xs mt-0.5 max-w-[200px]">Review past invoices and payment proofs.</p>
                 </div>
